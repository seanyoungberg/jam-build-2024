...
resource "aws_subnet" "this" {
  ...
  tags                    = merge(var.global_tags, each.value.local_tags, { Name = "${var.name_prefix}${each.value.name}" })
...
resource "aws_route_table" "this" {
  ...
  vpc_id           = var.vpc_id
  tags             = merge(var.global_tags, each.value.local_tags, { Name = "${var.name_prefix}${coalesce(each.value.route_table_name, each.value.name)}" })
